...
class R1EIDG_LoginController
{
    ...
    function handle_login_callback($request)
    {
        ...
        if (is_wp_error($token_verification_response))
            R1EIDG_LoginController::set_login_error_and_die(esc_html__("Token verification error", 'wp-mim-eidgateway-connect'));

        if ($token_verification_response['response']['code'] != 204)
            R1EIDG_LoginController::set_login_error_and_die(esc_html__("Invalid token", 'wp-mim-eidgateway-connect'));
        ...
        if(!$fiscal_number)
            R1EIDG_LoginController::set_login_error_and_die(esc_html__("Invalid fiscal code", 'wp-mim-eidgateway-connect'));
        ...
        if (empty($users))
            R1EIDG_LoginController::set_login_error_and_die(esc_html__("The user is not registered.", 'wp-mim-eidgateway-connect'));
